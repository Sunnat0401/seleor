...
class MailService {
  ...
  async sendOtpMail(email) {
    ...
    await otpModel.create({email , otp : hashedOtp , expireAt : Date.now() + 1 * 60 * 10000 })
    await this.transporter.sendMail({
        ...
        <p><strong>Note:</strong> Do not share this OTP width anyone for security reasons</p>
        `
  ...
  //   if(!record)  return {failure  : "Reacord not found"}
  //   const lastRecord = record[record.length-1]
  //   if(!lastRecord) return {failure : "Reacord not found "}
